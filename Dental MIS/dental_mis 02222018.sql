...
INSERT INTO `audit_trail` VALUES (1,'Patient Create Test Qwe','1','2018-02-22 01:13:54'),(2,'Patient Edit Test Qwea','1','2018-02-22 01:14:25'),(3,'Procedure Type Create Extraction','1','2018-02-22 01:17:16'),(4,'Procedure Type Edit Extraction','1','2018-02-22 01:17:28');
...
INSERT INTO `procedure_type` VALUES (6,'Extraction',500,'Full Payment',1,0,'1','2018-02-22 01:17:16','1','2018-02-22 01:17:28');
...
INSERT INTO `secret_question` VALUES (1,'What is your favorite color?','Admin','2018-02-19 02:56:07',NULL,NULL);
...
INSERT INTO `user` VALUES (1,1,1,'admin','password','red','Admin','User',1,'Admin',NULL,NULL,NULL),(2,2,1,'secretary','password','blackq','Secretary','Account',1,'Admin User','2018-02-19 03:07:41','Admin User','2018-02-19 05:55:41');
...
BEGIN
	IF `@from` <> '' OR `@to` <> '' THEN
		SELECT 
			...
			`audit_trail`.`created_date`
		FROM `dental_mis`.`audit_trail`
		WHERE `audit_trail`.`created_date` BETWEEN `@from` AND `@to`
		ORDER BY `audit_trail`.`created_date` ASC;
	ELSE
    		SELECT 
			`audit_trail`.`id`,
			`audit_trail`.`action`,
...
BEGIN
		...
		SELECT `procedure_type`.`id`,
			...
			`procedure_type`.`updated_date`
		FROM `dental_mis`.`procedure_type`
        WHERE `procedure_type`.`id` NOT IN(
...
BEGIN
    SELECT @r := @r+1 `index` , z.* FROM(
		...
		WHERE DATE_FORMAT(`schedule`.`start_time`, '%Y-%m-%d') = `@date` AND
			  Not `schedule`.`is_deleted`
...
BEGIN
    SELECT @r := @r+1 `index` , z.* FROM(
		SELECT `schedule`.`id`,
			...
			`schedule`.`updated_date`
		FROM `dental_mis`.`schedule`
		WHERE DATE_FORMAT(`schedule`.`start_time`, '%Y-%m-%d') BETWEEN `@from` AND `@to` AND
			  Not `schedule`.`is_deleted`
        ORDER BY `schedule`.`start_time` ASC
	)z, (SELECT @r:=0)y;
END ;;
...
BEGIN
	IF `@search_text` <> '' THEN
		SELECT 
			...
			`secret_question`.`created_by`,
			`secret_question`.`created_date`,
			`secret_question`.`updated_by`,
			`secret_question`.`updated_date`
		FROM `dental_mis`.`secret_question`
		WHERE (`secret_question`.`question` LIKE CONCAT('%', `@search_text`, '%'));
	ELSE
		SELECT 
			`secret_question`.`id`,
			`secret_question`.`question`,
...
-- Dump completed on 2018-02-22  1:29:23
